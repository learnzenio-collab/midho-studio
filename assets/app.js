...
function bindCtas(site){
  const lynk=site.lynk||"https://lynk.id/midhostudio";
  $("[data-whatsapp]").forEach(el=>el.onclick=e=>{if(site.whatsapp){el.href="https://wa.me/"+site.whatsapp;el.target="_blank";el.rel="noopener";return}e.preventDefault();toast("Nomor WhatsApp belum diatur admin.")});
  $("[data-lynk]").forEach(el=>{el.href=lynk;el.target="_blank";el.rel="noopener";el.onclick=null});
}
...
async function render(){
  ...
  try{
    ...
    const ig=$("#publicInstagram");if(ig){ig.textContent=s.instagram||"—";ig.href=s.instagram||"#"} const fig=$("#footerInstagram");if(fig){fig.href=s.instagram||"#"} const fem=$("#footerEmail");if(fem){fem.href=s.email?"mailto:"+s.email:"#"}
